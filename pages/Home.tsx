...
const Home: React.FC<HomeProps> = ({
  ...
  const sortedMatches = useMemo(() => {
    ...
    // FILTER OUT HIDDEN LIQUIDITY MATCHES
    const visibleMatches = matches.filter(
      (m) => m.homeTeam !== "Liquidity" && m.awayTeam !== "Pool"
    );
    ...
    unpredictedFuture.sort(
      (a, b) => Number(a.kickoffTime) - Number(b.kickoffTime)
    );
    predicted.sort((a, b) => Number(a.kickoffTime) - Number(b.kickoffTime));
    unpredictedPast.sort(
      (a, b) => Number(b.kickoffTime) - Number(a.kickoffTime)
    );
          ...
          {currentAccount && (
            <div className="bg-[#1A1A1A] rounded-full px-4 py-1.5 flex items-center gap-2 border border-white/10">
              ...
              <span className="text-xs font-mono font-bold text-white">
                {parseFloat(userBalance).toFixed(2)} CELO
              </span>
            </div>
          )}
        </div>
        ...
        {sortedMatches.map((match) => {
          ...
          return (
            <div
              key={match.id}
              onClick={() =>
                !isClosed && !alreadyPredicted && onSelectMatch(match)
              }
              className={`bg-[#161616] border border-white/5 rounded-3xl p-5 relative overflow-hidden transition-all
                ${
                  !isClosed && !alreadyPredicted
                    ? "active:scale-[0.98] hover:border-white/10"
                    : ""
                }
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex flex-col items-center px-4">
                  {alreadyPredicted && myPred ? (
                    ...
                    <span className="text-gray-700 text-sm font-black bg-gray-900 px-2 py-1 rounded">
                      VS
                    </span>
                  )}
                </div>
